...
func (a *AuthServer) ValidateSAMLAuthCallback(q url.Values) (*SAMLAuthResponse, error) {
	...
	if len(req.PublicKey) != 0 {
		certTTL := 3600 * time.Second // req.CertTTL // utils.MinTTL(utils.ToTTL(a.clock, ident.ExpiresAt), req.CertTTL)
		allowedLogins, err := roles.CheckLogins(3600 * time.Second)
		if err != nil {
			log.Debugf("CheckLogins failed")
			return nil, trace.Wrap(err)
		}
		cert, err := a.GenerateUserCert(req.PublicKey, user.GetName(), allowedLogins, certTTL)
		if err != nil {
			log.Debugf("GenerateUserCert failed")
			return nil, trace.Wrap(err)
		}
		response.Cert = cert
		...
		authorities, err := a.GetCertAuthorities(services.HostCA, false)
		if err != nil {
			log.Debugf("GetCertAuthorities failed")
			return nil, trace.Wrap(err)
		}
		for _, authority := range authorities {
			response.HostSigners = append(response.HostSigners, authority)
