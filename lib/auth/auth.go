...
func NewAuthServer(cfg *InitConfig, opts ...AuthServerOption) *AuthServer {
	...
	as := AuthServer{
		...
		samlClients:                   make(map[string]*samlClient),
...
type AuthServer struct {
	...
	samlClients map[string]*samlClient
...
func (s *AuthServer) ExtendWebSession(user string, prevSessionID string) (services.WebSession, error) {
	...
	// any more without extra logic for renewal with external OIDC/SAML provider
...
func (s *AuthServer) UpsertSAMLConnector(connector services.SAMLConnector, ttl time.Duration) error {
	return s.Identity.UpsertSAMLConnector(connector, ttl)
}
...
func (s *AuthServer) SendSAMLMetadata(connector services.SAMLConnector) ([]byte, error) {
	samlClient, err := s.getSAMLClient(connector)
	if err != nil {
		return nil, trace.Wrap(err)
	}
	buf, err := xml.MarshalIndent(samlClient.ServiceProvider.Metadata(), "", "  ")
	if err != nil {
		return nil, trace.Wrap(err)
	} else {
		return []byte(buf), nil
...
func (a *AuthServer) createSAMLUser(connector services.SAMLConnector, claims utils.TokenClaims) error {
	roles := connector.MapClaims(claims)
...
func (a *AuthServer) ValidateSAMLAuthCallback(q url.Values) (*SAMLAuthResponse, error) {
	...
	claims.ExpiresAt = assertion.Conditions.NotOnOrAfter.Unix()
	claims.NotBefore = assertion.Conditions.NotBefore.Unix()
	...
	log.Debugf("[IDENTITY] %v expires at: %v", claims.Attributes["email"][0], claims.ExpiresAt)
	...
	response.Username = user.GetName()
	log.Debugf("[IDENTITY] username %v", response.Username)
	...
	sessionTTL := roles.AdjustSessionTTL(utils.ToTTL(a.clock, time.Time(assertion.Conditions.NotOnOrAfter)))
	bearerTokenTTL := utils.MinTTL(BearerTokenTTL, sessionTTL)
	...
	if len(req.PublicKey) != 0 {
		certTTL := utils.MinTTL(utils.ToTTL(a.clock, time.Time(assertion.Conditions.NotOnOrAfter)), req.CertTTL)
		allowedLogins, err := roles.CheckLogins(certTTL)
		if err != nil {
			return nil, trace.Wrap(err)
		}
...
func samlConfigsEqual(a, b samlsp.Options) bool {
	...
	if a.IDPMetadataURL != b.IDPMetadataURL {
		return false
	}
